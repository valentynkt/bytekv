...
size_t db_active_sweep(db_t *db, size_t sample_size) {
  ...
  if (sample_size > db->expires->used)
    sample_size = db->expires->used;

  size_t picks[sample_size];
  size_t picked = 0;
  size_t cap = db->expires->size;

  while (picked < sample_size) {
    size_t idx = arc4random_uniform((uint32_t)cap);
    if (db->expires->table[idx] != NULL)
      picks[picked++] = idx;
  }
